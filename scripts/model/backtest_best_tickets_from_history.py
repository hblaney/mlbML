"""Build best-ticket ledger from existing walk-forward prediction history (fast path)."""
...
HISTORY_PATH = Path(__file__).resolve().parents[2] / "public" / "prediction-history.json"
...
def build_from_rows(rows: list[dict], start: date, end: date, store: HistoricalOddsStore) -> dict:
    ...
    return {
        ...
        "method": "strict_walk_forward_from_prediction_history",
        "model_version": rows[0].get("modelVersion", "unknown") if rows else "unknown",
...
def main() -> None:
    ...
    if HISTORY_PATH.exists():
        ...
        rows = [
            row
            for row in history.get("predictions", [])
            if start.isoformat() <= row["date"] <= end.isoformat()
        ]
        if rows:
            payload = build_from_rows(rows, start, end, store)
            payload["odds_metadata"] = odds_metadata
            OUTPUT_PATH.write_text(json.dumps(payload, indent=2))
            print(f"wrote={OUTPUT_PATH} from_history rows={len(rows)}")
